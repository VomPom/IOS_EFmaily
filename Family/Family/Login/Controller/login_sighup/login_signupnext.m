...
@interface login_signupnext ()
@property (nonatomic,strong) UIButton *receiveCheckNumButton;

@end
@implementation login_signupnext
...
- (void)viewDidLoad {
    ...
    //将触摸事件添加到当前view
    NSLog(@"%@",self.next_phone.text);
    [self.view addGestureRecognizer:tapGestureRecognizer];
    
    
    _receiveCheckNumButton = [[UIButton alloc] initWithFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width - 100)/2,140, 100, 40 )];
...
-(void)keyboardHide:(UITapGestureRecognizer*)tap{
    [self.next_num resignFirstResponder];
}


- (IBAction)nexttoset {
    login_set *vc = [[login_set alloc]init];
